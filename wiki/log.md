...
2026-04-27: [architecture] corpus.sh UX: --max-wait flag (default 1800s), progress shows "Completed: X/N (in pipeline: Y)", poll loop wrapped in set +e/set -e for transient-failure robustness
2026-04-27: [architecture] GeminiEmbedder.embed/embed_batch: retry once on asyncio.TimeoutError before propagating — handles transient TCP stalls to Vertex AI embed API
2026-04-27: [algorithms] corpus: _process_document_pipeline wraps inner pipeline with try/except — logs full traceback + filename on failure, writes pipeline_done_at with pipeline_error="failed" so status check completes instead of hanging until MAX_WAIT; structural edges now logs "start" and "complete" separately; CorpusStatusResponse gains failed field; is_complete = completed+failed >= total
2026-04-27: [algorithms, api.md] chunk creation parallelized — Phase A: all chunk nodes created via asyncio.gather; Phase B: has_chunk + next_chunk + mentioned_in edges in one gather; added POST /v1/ingest/corpus/{corpus_id}/status to api.md (was missing)
2026-04-27: [architecture] GeminiLLM._generate retry once on TimeoutError (same pattern as embed); safety filters set to BLOCK_NONE for all harm categories (code content trips defaults); "likely safety filter" log clarified — TimeoutError now logs distinctly from other failures
